...
def preprocess_data(
    ...
    for stock in list_of_stocks:
        ...
        # Save to CSV
        df.to_csv("data/" + stock + "_Processed_bb_rsi_stoch_nick.csv", index=False)
        list_of_stocks_processed.append(stock + "_Processed_bb_rsi_stoch_nick")
...
def logic(
    ...
    training_period = v1
    stop_loss_percent = v2
    today = len(lookback) - 1

    if today < training_period:
        return

    if account.long_or_short == "long" and lookback["close"][today] <= account.stoploss:
    ...
    if (
        ...
        if (
            ...
            close_and_enter(
                "long", account, lookback, today, stop_loss_percent=stop_loss_percent
            )
        ...
        if (
            ...
            close_and_enter(
                "short", account, lookback, today, stop_loss_percent=stop_loss_percent
            )
...
def close_and_enter(
    ...
    *,
    ...
    if enter and account.buying_power > 0:
        ...
        if pos == "long":
            # Set stoploss
            account.stoploss = lookback["close"][today] * (1 - stop_loss_percent)
        else:
