...
def calculate_time_slice(
    ...
    monthsagostart = (
        today - start_datetime_object
    ).days // 30  # Calculates the number of months between today and start date
    if monthsagostart < 1:  # If start date is before today, set start date to today
        monthsagostart = 0
        timeslicestart = "year1month1"
    elif (
        monthsagostart >= 24
    ):  # If start date is more than 2 years ago, set to 2 years ago
        monthsagostart = 24
        timeslicestart = "year2month12"
    else:
        yearsagostart = (
            monthsagostart // 12
        ) + 1  # Calculates the number of years between today and start date
        if yearsagostart == 2:
            monthsagostart = (
                monthsagostart - 12
            )  # If the start date is in the year "2",
...
def get_intraday_extended(
    symbol, start_date, end_date, interval, combine=True
    ...
    startindex, endindex, start_date, end_date = time_controller(
        start_date, end_date
    )  # Generate start and end index for retrieving data
